...
class ann_reLU:
    ...
    # def weightUpdate(arg, weightLama, weight, momentum, learn, delta, aktivasi,output,label):  # menghitung perubahan weight#aktivasi=input
    ...
    #     weightU = weight + ((output - label)*learn*np.dot(aktivasi,(1-aktivasi)))*(momentum * weightLama)
    #     weightLama = np.sum((output - label)*learn*np.dot(aktivasi,(1-aktivasi)))*(momentum * weightLama)
    #     return np.array(weightU), np.array(weightLama)

    def weightUpdate(arg, weightLama, weight, momentum, learn, aktivasi, input,output,label):  # menghitung perubahan weight#aktivasi=input

        #
        # weightU = weight +((-learn * np.dot(np.transpose(aktivasi), delta)) + (momentum * weightLama))
        # weightLama =np.sum(-learn * np.dot(np.transpose(aktivasi), delta))+ (momentum * weightLama)

        weightU = weight + np.dot(((output - label),np.dot((weightLama),np.dot(np.dot(aktivasi,(1-aktivasi)),input))
        weightLama = (np.sum(output - label)*(weightLama)*np.dot(np.dot(aktivasi,(1-aktivasi)),input))
        # print((np.sum(output - label)*(momentum * weightLama)*learn*np.dot(np.dot(aktivasi,(1-aktivasi)),input)).shape)
        return np.array(weightU), np.array(weightLama)
    ...
    def pengujian(arg, test, wInput, biasHidden1, wHidden1, biasHidden2, wHidden2, biasO, testLabel):#pengujian
        ...
        aktivasiOP = t.layerAktivasiOutput(aktivasiH2P, wHidden2, biasO)
...
for i in range(t.epoch):
    ...
    wHidden2, wHidden2Lama = t.weightUpdate(wHidden2Lama, wHidden2, t.momentum, t.learning,aktivasiO, aktivasiH2,aktivasiO,trainLabel)
    wHidden1, wHidden1Lama = t.weightUpdate(wHidden1Lama, wHidden1, t.momentum, t.learning, aktivasiH2,aktivasiH1,aktivasiO,trainLabel)
    wInput, wInputLama = t.weightUpdate(wInputLama, wInput, t.momentum, t.learning, aktivasiH1, train,aktivasiO,trainLabel)
    # weightUpdateInput(weightLama, weight, momentum, learn, delta, aktivasi, output, label, train)
